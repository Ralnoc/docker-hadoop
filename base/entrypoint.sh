...
function configurePropertyFile() {
    local path=$1
    local module=$2
    local envPrefix=$3
    ...
    for c in `printenv | perl -sne 'print "$1 " if m/^${envPrefix}_(.+?)=.*/' -- -envPrefix=$envPrefix`; do
        ...
        value=${!var}
        echo " - Setting $name=$value"
        addPropertyFileLine $path $name "$value"
    done
...
configurePropertyFile /opt/spark/conf/spark-defaults.properties spark SPARK_DEFAULT_CONF
